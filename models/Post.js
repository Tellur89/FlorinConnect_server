...
class Post {
  constructor({
    ...
    this.date_created = new Date(date_created).toLocaleDateString("en-GB", {
  ...
  static async showByCategory(type) {
    const response = await db.query(
      "SELECT * FROM posts WHERE category = $1 ORDER BY date_created;",
      [type]
    );
    if (response.rows.length === 0) {
      throw new Error("No posts available.");
  ...
  static async showByDate(date) {
    const response = await db.query(
      "SELECT * FROM posts WHERE date_created::date = $1::date;",
      [date]
    );
    if (response.rows.length < 1) {
  ...
  static async showBetweenDates(startDate, endDate) {
    const response = await db.query(
      "SELECT * FROM posts WHERE date_created BETWEEN $1::date AND $2::date ORDER BY date_created;",
      [startDate, endDate]
    );
    if (response.rows.length === 0) {
  ...
  static async showByWord(word) {
    const response = await db.query(
      "SELECT * FROM posts WHERE LOWER (content) LIKE $1 OR LOWER (title) LIKE $1;",
      ["%" + word + "%"]
    );
    if (response.rows.length === 0) {
  ...
  static async showByOpen() {
    const response = await db.query(
      "SELECT * FROM posts WHERE open = TRUE ORDER BY date_created;"
    );
  ...
  static async showByCompleted() {
    const response = await db.query(
      "SELECT * FROM posts WHERE completed = TRUE ORDER BY date_created;"
    );
  ...
  static async showByAccepted() {
    const response = await db.query(
      "SELECT * FROM posts WHERE accepted = TRUE ORDER BY date_created;"
    );
  ...
  static async create(data) {
    ...
    const response = await db.query(
      "INSERT INTO posts (title, content, category, date_created, image_url) VALUES ($1,$2,$3,$4,$5) RETURNING *;",
      [title, content, category, date_created, image_url]
    );
